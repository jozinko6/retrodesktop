...
use crate::{domain::{DetectionCandidate, DetectionResult}, error::{AppError, AppResult}};

fn extension_system(extension: &str) -> Option<(&'static str, f32)> {
    Some(match extension {
        "nes" => ("nes", 0.92), "sfc" | "smc" => ("snes", 0.88),
        "gb" => ("gb", 0.91), "gbc" => ("gbc", 0.91), "gba" => ("gba", 0.92),
        "n64" | "z64" | "v64" => ("n64", 0.88), "nds" => ("nds", 0.92),
        "md" | "gen" => ("genesis", 0.82), "sms" => ("mastersystem", 0.9),
        "gg" => ("gamegear", 0.9), "a26" => ("atari2600", 0.9),
        "a52" => ("atari5200", 0.9), "a78" => ("atari7800", 0.9), "lnx" => ("lynx", 0.9),
        "pbp" => ("ps1", 0.72), "cso" => ("psp", 0.82), "gcz" | "rvz" | "wbfs" => ("gamecube", 0.72),
        "adf" => ("amiga", 0.88), "d64" => ("c64", 0.88), "jsdos" => ("dos", 0.9),
        _ => return None,
    })
}
...
pub fn detect(path: &Path) -> AppResult<DetectionResult> {
    ...
    if let Some((system, confidence)) = extension_system(&extension) {
        ...
        candidates.push(DetectionCandidate { system_id: system.into(), confidence });
    }
    ...
    if path.is_file() {
        ...
        if count >= 4 && &header[..4] == b"NES\x1a" {
            ...
            candidates.push(DetectionCandidate { system_id: "nes".into(), confidence: 0.99 });
        }
        if count >= 4 && &header[..4] == b"\x24\xff\xae\x51" {
            evidence.push("Nintendo 64 big-endian header".into());
            candidates.push(DetectionCandidate { system_id: "n64".into(), confidence: 0.99 });
        }
    }
    ...
    Ok(DetectionResult {
        system_id: best.filter(|item| item.confidence >= 0.7).map(|item| item.system_id.clone()),
...
pub fn parse_cue(contents: &str) -> Vec<String> {
    contents.lines().filter_map(|line| {
        ...
        if !trimmed.to_ascii_uppercase().starts_with("FILE ") { return None; }
...
mod tests {
    ...
    fn parses_multibin_cue() {
        let files = parse_cue("FILE \"Track 01.bin\" BINARY\n TRACK 01 MODE2/2352\nFILE \"Track 02.bin\" BINARY");
